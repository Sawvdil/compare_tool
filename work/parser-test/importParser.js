...
const normalizeLabelText = (label) => label
    .trim()
    .replace(/[：:：=，,。；;、()[\]（）【】{}<>《》"'“”‘’\s]/g, '');
export const normalizeFieldLabel = (label) => {
    const cleanedLabel = normalizeLabelText(label);
    if (!cleanedLabel)
        return '';
    const upperLabel = cleanedLabel.toUpperCase();
    if (['价格', '售价', '发售价', '首发价', 'MSRP', '官方售价'].includes(upperLabel))
        return '价格';
    if (['焦段', '焦距', '变焦范围', '等效焦段', '等效焦距'].includes(cleanedLabel)) {
        return cleanedLabel.includes('等效') ? '等效焦段' : '焦段';
    }
    if (['光圈', '最大光圈', '恒定光圈'].includes(cleanedLabel))
        return '最大光圈';
    if (['重量', '镜头重量', '机身重量', '净重'].includes(cleanedLabel))
        return '重量';
    if (['尺寸', '体积', '长度', '直径', '外形尺寸'].includes(cleanedLabel))
        return '尺寸';
    if (['防抖', '光学防抖', '镜头防抖', '是否防抖'].includes(cleanedLabel))
        return '防抖';
    if (['OSS', 'OIS', 'VC'].includes(upperLabel))
        return '防抖';
    ...
    if (upperLabel === 'AF')
        return '对焦';
    if (['画质', '锐度', '成像', '成像质量', '解析力'].includes(cleanedLabel))
        return '画质';
    if (['滤镜口径', '口径', '滤镜尺寸'].includes(cleanedLabel))
        return '滤镜口径';
    if (['最近对焦距离', '最近拍摄距离', '最近距离'].includes(cleanedLabel))
        return '最近对焦距离';
    if (['放大倍率', '最大放大倍率', '近摄倍率'].includes(cleanedLabel))
        return '最大放大倍率';
    if (['优点', '优势', '亮点', '好处', '卖点'].includes(cleanedLabel))
        return '优点';
    if (['缺点', '劣势', '不足', '短板', '问题'].includes(cleanedLabel))
        return '缺点';
    if (['备注', '说明', '其他', '补充'].includes(cleanedLabel))
        return '备注';
    return cleanedLabel;
};
export const mergeDuplicateFields = (fields) => {
...
const normalizeLine = (line) => line.replace(/^[\s*+\-•·、\d.）)]+/, '').trim();
...
const parseStructuredLines = (lines, fields) => {
    const remarks = [];
    lines.forEach((line) => {
...
const parseKeywordDescription = (text, fields) => {
    const normalized = text.replace(/\n+/g, '，');
    const keywordLabels = [...knownLabels].sort((left, right) => right.length - left.length);
    const keywordPattern = new RegExp(`(${keywordLabels.join('|')})(?:\\s*(?:是|为|约|大约|：|:|=|-|—|–))?\\s*([^，,。；;\\n]+)`, 'gi');
...
const guessProductName = (firstLine, fullText) => {
    const cleanFirstLine = normalizeLine(firstLine);
    if (!cleanFirstLine || isFieldLine(cleanFirstLine))
        return '';
...
export const parseImportedText = (text) => {
    ...
    const plainRemarks = remarks.filter((remark) => remark !== productName &&
        !isFieldLine(remark) &&
